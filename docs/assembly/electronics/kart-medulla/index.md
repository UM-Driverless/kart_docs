...
| 35 | 3 | 3 | BUZZER → compressor PWM | Digital Out | **Repurposed** to `CMD_COMPRESSOR_PWM` — drives the EBS air-compressor MOSFET gate (terminal CN8.2); the buzzer was dropped. Strap pin (JTAG src select), idle-high at boot is acceptable. See the callout below. |
...
!!! info "Two repurposed terminals on the current board"
    ...
    - **CN5.2 / GPIO 1 — steering-angle PWM (was `PRESSURE_3`).** The Pressure-3 ADC channel is retired; the terminal now reads the steering-angle sensor's **single-wire PWM output — the wheel angle is encoded in the PWM duty cycle** — decoded on the ESP32-S3 with the MCPWM capture peripheral (edge timing), not read as an analog voltage. **Board rework: remove R10 only** (the pulldown to GND) — keep R8 + R9 (20 kΩ series). Removing R9 would break the CN5.2→GPIO 1 path, so the older "remove R9 + R10" note is wrong; the live netlist is `CN5.2 —[R8 10k]— node —[R9 10k]— GPIO 1 —[R10 10k]— GND`. The 20 kΩ series is too high-impedance for the ADC, so this pin reads digital PWM, not analog. The current steering sensor (AS5600) still reads over I²C; this PWM path is for the planned MT6701 — see [Angle Sensor](../../steering/sensor/index.md) and [Wiring](../wiring.md).
...
!!! note "Module suffix: N8R2 preferred; N8R8 still works"
...
!!! danger "Module suffix: N8R2 is the ordered part — octal-PSRAM variants (R8) are tolerated, never preferred"
